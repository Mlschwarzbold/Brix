...
// returns 0 on success, -1 on NOT_LOC_MSG, -2 on INVALID_IP, -3 on INVALID_PORT, -4 on INVALID_FORMAT, -5 for NULL return pointers
int extract_ip_and_port_from_response(char* response, char* return_ip, int* return_port){
    ...
    if (port <= 0 || port > 65535) { // server can be on well known ports, i guess
