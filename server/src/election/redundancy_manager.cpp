...
RedundancyManager::RedundancyManager() {
    ...
    snprintf(coord_announcement_message, sizeof(coord_announcement_message),
             "CRD %u END", id);
    ...
    // pthread_create(&election_listener_thread, NULL,
    // &RedundancyManager::start_election_waiting_server, (void *)port_ptr);
    // pthread_create(&election_listener_thread, NULL,
    // &RedundancyManager::start_election_waiting_server, (void *)NULL);
    // pthread_detach(election_listener_thread);

    // start election
    // int *election_port_ptr = new int(4000);
    // pthread_create(&election_thread, NULL,
    // &RedundancyManager::start_election, (void *)election_port_ptr);
...
void RedundancyManager::election_result_switch() {
    ...
    // If we got here, no answers or announcements were received, we become
    // coordinator
    std::cout << MAGENTA << "[ELECTION MANAGER]: I am the new coordinator!"
              << RESET << std::endl;
    is_coordinator = true;
    election_in_progress = false;
...
bool RedundancyManager::is_valid_answer(char *buffer, int n) {
    ...
    std::string tag, end;
    if ((iss >> tag >> responder_id >> end) && tag == "ANS" && end == "END" &&
        responder_id != id) { // not valid if from self
        // valid
...
bool RedundancyManager::is_valid_coord_announcement(char *buffer, int n) {
    ...
    std::istringstream iss(buffer);
    std::string tag, end;
    if ((iss >> tag >> coordinator_id >> end) && tag == "CRD" && end == "END" &&
        coordinator_id != id) { // not valid if from self
        // valid
...
bool RedundancyManager::is_valid_election_message(char *buffer, int n) {
    ...
    if (n <= 0) {
        // std::cout << BOLD << "invalid election message n <= 0" << RESET <<
        // std::endl;
        return false;
    }
    std::istringstream iss(buffer);
    std::string tag, end;
    if ((iss >> tag >> sender_id >> end) && tag == "ELE" && end == "END" &&
        sender_id != id) { // not valid if from self
        // valid
        // std::cout << BOLD << "valid election message from id " << sender_id
        // << RESET << std::endl;
        last_received_id = sender_id;
        return true;
    } else {
        // invalid
        // std::cout << BOLD << "invalid election message format" << RESET <<
        // std::endl;
        return false;
    }
}
...
void RedundancyManager::start_election_thread() {
    ...
    pthread_create(&election_thread, NULL, &RedundancyManager::start_election,
                   (void *)NULL);
    pthread_detach(election_thread);
}
